...
- 背景透明度改用 SwiftBar 官方支持的百分比子菜单，不再依赖无法显示的 `type=slider` 属性。
- 透明度调整、主题切换和图片主题写入共用锁，避免并发操作以旧的 `theme.json` 覆盖新主题。
- 主题写入锁记录唯一所有权令牌；旧进程恢复时只能释放自己的锁，不会误删后来创建的新锁。
- 图片加载和预设切换使用唯一的不可变图片名，确保图片先就绪、`theme.json` 最后提交，不再出现新图片配旧配置的中间状态。
- `--reset-demo` 现在会原子恢复内置主题和示例图片，不再删除活动主题后留下缺失的 `theme.json`。
